...
import com.careful.HyperFVM.databinding.ActivityAuxiliaryList10Binding;
import com.careful.HyperFVM.databinding.ActivityAuxiliaryList10EffectBinding;
import com.careful.HyperFVM.databinding.CardCardDataAuxiliaryList10Binding;
...
public class AuxiliaryList10Activity extends BaseActivity {
    // 顶部栏滚动联动的状态保存键与渐变区间
    ...
    private CardCardDataAuxiliaryList10Binding cardListBinding;// 增幅名单卡片列表（普通/流光两种布局的 include 同 id 同类型，合并后共用同一引用）
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (HyperFVMApplication.isContentDynamicBackgroundEnabled()) {
            ActivityAuxiliaryList10EffectBinding effectBinding = ActivityAuxiliaryList10EffectBinding.inflate(getLayoutInflater());
            cardListBinding = Objects.requireNonNull(effectBinding.cardCardDataAuxiliaryList10);
            setContentView(effectBinding.getRoot());
        } else {
            ActivityAuxiliaryList10Binding normalBinding = ActivityAuxiliaryList10Binding.inflate(getLayoutInflater());
            cardListBinding = Objects.requireNonNull(normalBinding.cardCardDataAuxiliaryList10);
