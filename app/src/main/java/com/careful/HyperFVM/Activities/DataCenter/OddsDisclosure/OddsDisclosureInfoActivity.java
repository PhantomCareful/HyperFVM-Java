...
public class OddsDisclosureInfoActivity extends BaseActivity {
    // 顶部栏滚动联动的状态保存键与渐变区间
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_odds_disclosure_info);

        // 初始化各种装饰效果
        initDecoration();

        LinearLayout container = findViewById(R.id.odds_disclosure_item_container);

        // 取出入口页传入的概率表数据；Parcel 反序列化得到的数组运行时类型为 Parcelable[]，需逐项转换回 OddsItem
        Parcelable[] parcelables = getIntent().getParcelableArrayExtra("oddsDisclosureInfo");
        if (parcelables != null) {
            OddsItem[] array = new OddsItem[parcelables.length];
            for (int i = 0; i < parcelables.length; i++) {
                array[i] = (OddsItem) parcelables[i];
            }

            // 加载概率表数据
            OddsDisclosureHelper.loadOddsDisclosureInfo(this, container, array);
        }
    }
    ...
    private void initDecoration() {
        ...
        TextView topBarBottom = findViewById(R.id.topBarBottom);
        ...
        nestedScrollUtil = NestedScrollUtil.attach(findViewById(R.id.scrollView), topBarBottom, topBar, blurViewTopBar, TOP_BAR_FADE_RANGE_DP);
