...
import com.careful.HyperFVM.databinding.CardCardDataAuxiliaryList3Binding;
...
public class AuxiliaryList3Activity extends BaseActivity {
    ...
    private CardCardDataAuxiliaryList3Binding cardListBinding;// 增幅名单卡片列表（普通/流光两种布局的 include 同 id 同类型，合并后共用同一引用）
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (HyperFVMApplication.isContentDynamicBackgroundEnabled()) {
            ActivityAuxiliaryList3EffectBinding effectBinding = ActivityAuxiliaryList3EffectBinding.inflate(getLayoutInflater());
            cardListBinding = Objects.requireNonNull(effectBinding.cardCardDataAuxiliaryList3);
            setContentView(effectBinding.getRoot());
        } else {
            ActivityAuxiliaryList3Binding normalBinding = ActivityAuxiliaryList3Binding.inflate(getLayoutInflater());
            cardListBinding = Objects.requireNonNull(normalBinding.cardCardDataAuxiliaryList3);
