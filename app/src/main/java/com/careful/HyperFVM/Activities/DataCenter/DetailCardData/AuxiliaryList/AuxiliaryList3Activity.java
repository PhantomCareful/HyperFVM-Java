...
import com.careful.HyperFVM.databinding.ActivityAuxiliaryList3Binding;
...
public class AuxiliaryList3Activity extends BaseActivity {
    // 顶部栏滚动联动的状态保存键与渐变区间
    private static final String STATE_SCROLL_Y = "state_auxiliary_list3_scroll_y";
    private static final String STATE_SCROLL_Y_PAD_1 = "state_auxiliary_list3_scroll_y_pad_1";// PAD 左栏（scrollView1 大图栏）滚动位置保存键：PAD 不接入联动，但 ScrollView 不自存滚动状态，需重建后恢复
    private static final String STATE_SCROLL_Y_PAD_2 = "state_auxiliary_list3_scroll_y_pad_2";// PAD 右栏（scrollView2 名单栏）滚动位置保存键：同上
    private static final int TOP_BAR_FADE_RANGE_DP = 50;// 顶部模糊遮罩层完整显现的滚动区间（dp）

    private ActivityAuxiliaryList3Binding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_auxiliary_list3);

        // 初始化ViewBinding
        binding = ActivityAuxiliaryList3Binding.inflate(getLayoutInflater());
    ...
    private void initCardImages() {
        ...
        Objects.requireNonNull(binding.cardCardDataAuxiliaryList3).cardCardDataIndex954.cardDataIndex954.setOnClickListener(v -> CardDataHelper.selectCardDataByName(this, "霹雳马"));
        Objects.requireNonNull(binding.cardCardDataAuxiliaryList3).cardCardDataIndex955.cardDataIndex955.setOnClickListener(v -> CardDataHelper.selectCardDataByName(this, "归元马"));
        Objects.requireNonNull(binding.cardCardDataAuxiliaryList3).cardCardDataIndex958.cardDataIndex958.setOnClickListener(v -> CardDataHelper.selectCardDataByName(this, "萌海马"));
