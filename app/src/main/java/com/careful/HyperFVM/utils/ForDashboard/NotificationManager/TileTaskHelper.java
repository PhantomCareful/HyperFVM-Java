    ...
    public static void executeTileTasks(
        ...
        Runnable checkAndSend = () -> {
            if (giftDone[0] && dashboardDone[0]) {
                ...
                tileTaskNotificationManager.sendTileTaskNotification("执行完成🎉", combinedContent);
            }
        };

        tileTaskNotificationManager.sendTileTaskNotification("温馨礼包⏳", "正在领取温馨礼包，请稍候~");
        executeDailyTask.executeGiftTileTask(resultGift -> {
            giftResult[0] = resultGift;
            giftDone[0] = true;
            checkAndSend.run();

            tileTaskNotificationManager.sendTileTaskNotification("更新数据⏳", "正在更新仪表盘数据，请稍候~");
